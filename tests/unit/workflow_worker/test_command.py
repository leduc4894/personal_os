...
The three long-running worker processes (projection dispatcher, policy
preview, policy reconciliation) open PostgreSQL connections through
SQLAlchemy's psycopg async driver inside their coroutines. psycopg async
refuses the Windows Proactor loop that a bare ``asyncio.run`` selects on
win32, so every process entrypoint must run its coroutine on a
SelectorEventLoop runner — the same contract the API CLI commands follow.
On Unix the default loop is already selector-backed, so the assertions hold
everywhere.
"""
