"""Collection rules for the device-sync contract directory.
...
``poe device-sync-device-verification`` task or an ``-m`` expression that
names ``device_records``. The default marker expression already deselects
it, but the acceptance feature gate's fixed marker override
...
blocks the final child handoff, not every intermediate gate. When the marker
is explicitly requested the test runs and fails — never skips — on missing
evidence.
"""
