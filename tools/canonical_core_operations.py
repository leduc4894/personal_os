...
``phase-one-acceptance`` is registered now and lands in Task 15: the
placeholder runs its environment gate, then exits ``69`` with one safe
registered diagnostic whose exception-type token names the deferral.
"""
...
class DeferredToTask15(Exception):
    """The registered phase-one-acceptance placeholder; lands in Task 15."""
...
def main(
    ...
    *,
    ...
    except DeferredToTask15:
        _print_json(
            out_stream,
            {"result_code": "phase_one_acceptance_deferred_to_task_15", "state": "error"},
        )
        emit_emergency_internal_error(
            _OPERATIONS_SERVICE, resolution.context, DeferredToTask15(), stderr=err_stream
        )
        return int(CanonicalCoreExitCode.UNAVAILABLE)
    except TimeoutError:
