...
| 2026-09-01 | small-file | Rows that reached terminal state before the locator-clear fix still carry `normalized_locator` — all three terminal writers clear going forward, but the identical-replay early-return is intentionally non-mutating; needs a one-time guarded data remediation (null locator on terminal rows, keep fingerprints) | Before production activation | [handoff §deferred](2026-09-01-canonical-correctness-and-migration-hygiene.md) |
