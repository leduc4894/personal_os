...
| 2026-09-03 | device-sync | Apply-lane wedge: a mid-apply vault failure (`verify_temp · device_apply_vault_failed`) whose durable settle itself fails (`recovery · device_cursor_gap`) retries forever — the defensive bound landed (`7e82701`: three same-reason retries surface a readable blocked verdict); REMAINING is the deep fix (the run completing despite the unsettled action — per-action durable attempt counting, journal schema v10) | Before Child 9 operations acceptance | [robustness handoff](2026-09-03-manifest-restart-and-apply-robustness.md) |
