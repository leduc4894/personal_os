...
| 2026-09-03 | device-sync | Untitled-transit create→move→rename hard-stops the journal on the real stack and repair cannot restart it: first Repair sync blocks at `actions` with `device_cursor_gap`, the 2026-09-02 automatic recovery re-mints a fresh run whose checkpoint still cannot fit the apply lattice (`base_ack=1 < checkpoint=2`, run resting `applying` with a pending download), and a further Repair sync is refused at the start-stage binding check (`device_manifest_state_invalid`) leaving an abandoned `collecting` run — first live observation of the frozen-open-run premise | Before Child 9 operations acceptance | [handoff](2026-09-03-lifecycle-rejection-ring-live-readback.md) |
