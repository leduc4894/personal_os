...
| 14 | Acceptance-test polish batch (Task 15): vacuous E2E assertions on mock constants; offline-state whitelist docstring overclaim; only the login cookie pair serves as Set-Cookie sentinels; dead `_RETIRED_MASTER_KEY` assignment; grant-table ordering dependency (documented in-line in the key-rotation file, see the §5 ruling); reproduce script prints fewer stats than the gate; fixed accepted-login password constant; inert `re.MULTILINE` flag | Batched as one BACKLOG line (2026-08-16, authentication-acceptance-tests); tighten when the acceptance layer is next touched. The `StarletteDeprecationWarning` in gate output is library-level (raised by the `fastapi.testclient` import in test files only; the repo carries no `filterwarnings` policy) — triaged record-not-filter, revisit at the next fastapi/starlette bump |
| 15 | Stack workflows other than `authentication-acceptance.yml` lack the mutual project-name/guard consistency pins this branch added to its own workflow (pre-existing) | Own BACKLOG line (2026-08-16, ci-workflows pre-existing) as a distinct cross-workflow defect; add the same consistency pin to the remaining stack workflows when they are next touched |
