...
## ADR-020 — Projection coalesces latest-wins with a configurable quiet-window dwell

**Decision:** Sync/upload giữ hành vi tức thời: mọi version commit ngay và ghi projection intent ngay (outbox ADR-019) — không defer upload theo setting phía plugin. Consumer phía projection (Qdrant/Neo4j) coalesce theo source: batch claim gom các intent pending của cùng source và chỉ embed/ghi projection cho version hiện tại (latest-wins); intent của version bị vượt chốt kết cục closed `superseded` không tốn embedding/API call. Mỗi version commit đẩy `available_at` của source lùi thêm một quiet-window dwell cấu hình được (mặc định 10 phút, cấu hình workspace-level phía server/Web Admin, không phải plugin setting) — source đang được sửa liên tục thì projection chưa chạy, ngừng sửa đủ dwell mới chạy một lần cho bản cuối. Chi phí embedding cộng gộp với deterministic chunk identity (chỉ chunk đổi nội dung mới re-embed) và digest cache (revert miễn phí).

**Reason:** Trì hoãn upload phía plugin mở cửa sổ mất dữ liệu cục bộ và cửa sổ conflict đa thiết bị; trì hoãn ở projection layer không đổi sync contract, giữ nguyên mọi đảm bảo correctness (idempotency, base version, conflict) và hạ chi phí embedding về một lần embed các chunk thay đổi của bản cuối mỗi phiên soạn thảo. Quyết định ghi trước khi Phase 3/4 land để spec ingestion/retrieval nhận ràng buộc này từ đầu thay vì retrofit.

## ADR-021 — Bounded version retention with policy-driven canonical GC

**Decision:** Lịch sử version KHÔNG vô hạn theo mặc định. Mỗi workspace phải mang một retention policy tường minh — keep-last-N version mỗi source hoặc time window — cấu hình qua Web Admin (ADR-012); tham số mặc định thuộc về spec retention khi nó được viết. Canonical GC xóa object R2 và prune version rows theo policy, và LUÔN giữ: version current của mỗi source, open tombstone lineage (retained locator + version fingerprint cho identity proof), và retained sync-event window mà các active device cursor còn cần. Sync-event compaction collapse event phía sau retained window; thiết bị lạc hậu nhận `device_cursor_gap` và hội tụ qua manifest repair (ADR child 6) thay vì replay lịch sử cổ. Projection (Qdrant/Neo4j) rebuild từ canonical (ADR-004); lịch sử projection tự collapse cùng version bị prune (ADR-020). Compaction execution + canonical GC phải land **Before production activation** — trùng mốc backlog index hiện có; cho tới lúc đó hệ thống tích lũy full history ở trạng thái pre-retention.

**Reason:** Tích lũy vô hạn ở quy mô vault cá nhân lớn (100k note) tạo áp lực lên PostgreSQL (vài chục triệu row version/event/intent) lâu trước khi chi phí R2 đáng kể (~0,4$/tháng cho 25GB). Contract cursor-gap + manifest-repair của Child 6 được đúc chính xác để compaction an toàn; ghi quyết định bounded-retention ngay bây giờ ràng buộc spec later-retention từ đầu — thời điểm rẻ nhất, trước khi bất kỳ code retention nào tồn tại.
