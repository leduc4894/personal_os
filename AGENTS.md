...
- Live Obsidian Desktop journey dùng `wdio-obsidian-service` là gate bắt buộc khi task yêu cầu. Không được tự đánh dấu deferred hoặc thay bằng mock; phải chạy nó với cấu hình/credential test đã được loader chuẩn nạp và báo cáo chính xác prerequisite ngoài workspace nếu vẫn bị chặn. Mobile acceptance có thể defer, nhưng phải được ghi đúng MỘT dòng trong `docs/handoff/BACKLOG.md` kèm handoff nguồn, lý do, và `Implement by` là mốc kiểm chứng được; không được ghi Mobile là passed khi chưa có evidence vật lý.
