...
- `.local/e2e-totp-code.py` is a code producer only after a TOTP credential is active. Before any manual Desktop live journey, run `tools/obsidian_live_acceptance_bootstrap.py` against the exact disposable `knowledge-ci-*` project. If the helper reports no active credential, the bootstrap must enroll and activate TOTP through the approved Web HTTP flow, rerun the helper preflight, and publish policy. A missing active credential by itself is a bootstrap branch, not a BLOCKED or deferred verdict.
...
- Live Obsidian Desktop journey không dùng WDIO. Codex chuẩn bị project disposable `knowledge-ci-*`, bootstrap/fixture/policy, và hướng dẫn journey; operator thao tác trực tiếp trên một Vault test thật tách khỏi Vault dữ liệu hằng ngày, kết nối đúng project CI đó. Runbook phải nêu fixture an toàn, từng thao tác, expected state và evidence sanitize; evidence chỉ gồm outcome, reason token, count và timestamp — không content, path, token hoặc screenshot nhạy cảm. Codex xác minh API/diagnostics tại các checkpoint; manual evidence là gate bắt buộc, không được thay bằng mock hoặc tự đánh dấu deferred. Mobile acceptance có thể defer, nhưng phải được ghi đúng MỘT dòng trong `docs/handoff/BACKLOG.md` kèm handoff nguồn, lý do, và `Implement by` là mốc kiểm chứng được; không được ghi Mobile là passed khi chưa có evidence vật lý.
